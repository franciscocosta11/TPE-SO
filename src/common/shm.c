...
void* shm_create_map(const char *name, size_t size, int prot) {
    ...
    if (ftruncate(fd, (off_t)size) == -1) { perror("ftruncate"); close(fd); return NULL; }
    void *p = mmap(NULL, size, prot, MAP_SHARED, fd, 0);
    if (p == MAP_FAILED) { perror("mmap create"); close(fd); return NULL; }
    close(fd);
    return p;
}
...
void* shm_attach_map(const char *name, size_t *out_size, int prot) {
    ...
    if (p == MAP_FAILED) { perror("mmap attach"); close(fd); return NULL; }
